...
# NOISE TESTS
from nn_ood.utils.data_transforms import GaussianNoise, GaussianBlur, TransformedDataset

transforms = {
#     'gaussian_0': GaussianNoise(0),
#     'gaussian_1': GaussianNoise(1),
#     'gaussian_2': GaussianNoise(2),
#     'gaussian_3': GaussianNoise(3),
#     'gaussian_4': GaussianNoise(4),
    'blur_0': GaussianBlur(0),
    'blur_1': GaussianBlur(1),
    'blur_2': GaussianBlur(2),
    'blur_3': GaussianBlur(3),
    'blur_4': GaussianBlur(4),
}

def viz_transforms(idx=0, unc_model=None, model=None):
    num_plots = len(transforms) + 1
    fig, axes = plt.subplots(1,num_plots, figsize=[5*num_plots, 5], dpi=100)
    base_dataset = dataset_class(in_dist_splits[0])
    viz_dataset_sample(axes[0], base_dataset, idx=idx, unc_model=unc_model, model=model)
    
    for i, (name, transform) in enumerate( transforms.items() ):
        dataset = TransformedDataset(base_dataset, transform)
        viz_dataset_sample(axes[i + 1], dataset, idx=idx, unc_model=unc_model, model=model)
